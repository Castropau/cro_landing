...
// // Extract plain text body from Gmail message payload, fallback to HTML if no plain text found
...
//         const bodyText = getPlainTextFromPayload(full.data.payload) || "";
...
function removeQuotedText(plainText: string): string {
  const lines = plainText.split('\n');
  const cleanedLines = lines.filter(line => !line.startsWith('On ') && !line.startsWith('>'));
...
function getPlainTextFromPayload(payload?: GmailPayload): string {
  ...
  if (payload.mimeType === "text/plain" && payload.body?.data) {
    ...
    return removeQuotedText(plainText);  // Remove quoted text if necessary
  }
...
//   if (payload.mimeType === "text/plain" && payload.body?.data) {
//     const buff = Buffer.from(payload.body.data, "base64");
//     return buff.toString("utf-8").trim();
//   }
...
//   if (!email || !ticket_number) {
//     return NextResponse.json({ error: "Missing email or ticket_number" }, { status: 400 });
//   }
